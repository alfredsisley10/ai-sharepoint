...
const INSTRUCTIONS = [
  ...
  "To CHECK that the Outlook channel works, call test_outlook_channel: it drafts a code to the",
  "user THEMSELVES, they confirm it from their Drafts folder, and the draft is removed — nothing",
  "is sent. A channel test is a COMPLETE transaction: report the outcome and STOP. Never follow",
  "it with draft_communication or by asking for a recipient — composing a real message is a",
  "separate transaction that starts only from an explicit user request naming the recipient.",
