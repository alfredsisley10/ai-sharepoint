...
| Chat message fails to send (network error) | A corporate content proxy / DLP may be blocking a word in the message → open **Manage Proxy Avoid-List**, add the trigger word(s), and set `aiSharePoint.proxy.mode` to `defang` so future messages auto-adjust. When defang rewrites a message, a **🛡️ See what was changed** button appears showing exactly which words were altered (an invisible zero-width character is inserted so the proxy can't match — meaning is unchanged). |
| Connection fails behind a proxy / TLS-inspection | The extension auto-diagnoses this and shows the fix inline (trust the proxy root CA via `NODE_EXTRA_CA_CERTS` / OS trust store, set `http.proxy` for a 407, or ask IT to allowlist the host). Admin Guide §3. |
