...
- QUEUED (user, this session): (a) Confluence/Jira global catalog pre-cache — first-use offer, TTL expiry, continue?-checkpoints every N seconds while loading; (b) Communication Channels — Teams chat + Outlook mail connectors, draft-with-AI → user approves → send via Graph (ADR-0021-style gates); (c) Vertex AI Search connector (vertexaisearch.cloud.google, enterprise Gemini, SSO) for search/analysis.
