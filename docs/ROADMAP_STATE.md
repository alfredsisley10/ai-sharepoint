...
| Remaining §9.2 adapters (Intune, Databricks, AppDynamics, Azure/O365 consoles, Workday; Oracle excluded per ADR-0022; Splunk SHIPPED 0.10.0, ServiceNow 0.9.0, Power BI 0.8.0, Splunk Observability Cloud/SignalFx + Grafana 0.23.0, Aternity 0.135.0; Vertex AI Search shipped 0.7.0 then REVERTED 0.99.0 — ADR-0026) | Each needs live-instance validation; framework seam proven by 15 adapters |
...
- 2026-08-31: 0.135.0 — Riverbed Aternity connector (ADR-0049: aternity type over the OData REST API at `<account>-odata.aternity.com/aternity.odata/latest/`, dashboard↔OData SaaS endpoint derivation w/ on-prem passthrough, basic (OData REST API role) or pasted OAuth bearer auth — no new auth method, bare-table + JSON {table, filter, select, top, timeframe} query shapes w/ relative_time composition and $top always capped, honest no-free-text-search refusal, no item fetch (rows unaddressable), service-document table browse w/ curated fallback, ?table= default + $top=1/service-doc verify). On standard rails (lockout/caps/cache/wire/export-import). 1137 tests. NOTE: needs live-instance validation per register.
