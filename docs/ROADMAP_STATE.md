...
- 2026-06-11: 0.7.0 (part 2) IN PROGRESS — DB schema preload + AI semantic index: introspect INFORMATION_SCHEMA (mssql/pg/mysql) + Mongo field sampling into a local catalog; consent-gated Copilot indexing (names only, metered) producing semantic tags/synonyms (e.g. group_cio→ownership) so free-form questions ("records owned by X") hit the right columns; schema tool for chat. See ADR-0024 when landed.
