...
| Microsoft 365 Copilot (optional) | `graph.microsoft.com` (same host as site reads) — read-only `POST /copilot/retrieval` for grounded context; reuses the Microsoft 365 sign-in. Requires a **Microsoft 365 Copilot licence** and delegated **`Files.Read.All` + `Sites.Read.All`** consent (ADR-0034) |
