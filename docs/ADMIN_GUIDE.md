...
| Splunk (optional) | Your Splunk management endpoint (typically `:8089`) — read-only SPL via queued search jobs (rides the concurrency queue like Splunk Web, jobs always deleted afterwards), write/exfil commands blocked client-side, default 24 h window (ADR-0029) |
